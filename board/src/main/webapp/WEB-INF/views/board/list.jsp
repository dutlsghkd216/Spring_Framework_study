				...
				<div class="table-wrapper">
						...
						<tbody>
							<c:forEach var="board" items="${boardList}">
								<tr class="tBody">
									...
									<td class="title"><a
										href="/board/read${pageDTO.criteria.params }&bno=${board.bno}">${board.title}</a></td>
					...
					<form name="searchForm" action="/board/list">
						<div class="fields">
							<div class="field" style="text-align: center;">
								<select name="type">
									<option value=""
										${pageDTO.criteria.type == null  ? 'selected' : ' '}>검색기준</option>
									<option value="TCW"
										${pageDTO.criteria.type == 'TCW' ? 'selected' : ' '}>전체</option>
									<option value="T"
										${pageDTO.criteria.type == 'T' ? 'selected' : ' '}>제목</option>
									<option value="C"
										${pageDTO.criteria.type == 'C' ? 'selected' : ' '}>내용</option>
									<option value="W"
										${pageDTO.criteria.type == 'W' ? 'selected' : ' '}>작성자</option>
									<option value="TW"
										${pageDTO.criteria.type == 'TW' ? 'selected' : ' '}>제목
										또는 작성자</option>
									<option value="TC"
										${pageDTO.criteria.type == TC ? 'selected' : ' '}>제목
										또는 내용</option>
								</select> <input type="text" name="keyword" value="${pageDTO.criteria.keyword}"> <input type="hidden"
									name="pageNum" value="${pageDTO.criteria.pageNum }"> <a
					...
					<div style="text-align: center" class="big-width">
						...
						<c:if test="${pageDTO.next}">
							<a class="changePage" href="${pageDTO.endPage + 1 }"> <code>&gt;</code>
							</a>
						</c:if>
					</div>
...
<script>
	...
	$("a.changePage").on("click", function(e) {
		...
		//form태그 자식 요수 중 pageNum이 이름인 input태그 가져오기
